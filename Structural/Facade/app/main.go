...
// That’s why there’s a concept of the Facade pattern: a thing that lets the client work with dozens of components
// using a simple interface. The client only needs to enter the card details, the security pin, the amount to pay,
// and the operation type. The Facade directs further communications with various components without exposing the
// client to internal complexities.
...
func main() {
	fmt.Println()
	walletFacade := facade_lib.NewWalletFacade("abc", 1234)
	fmt.Println()
